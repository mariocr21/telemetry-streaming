...
watch(
    ...
    (newSensor) => {
        if (!newSensor) return;
        const targetValue = typeof newSensor.value === 'number' ? newSensor.value : 0;
...
<template>
    <div class="-py-2 -my-2 h-full w-full">
        <svg viewBox="0 0 200 160" class="h-full w-full">
            <!-- Fondo del velocímetro -->
            <circle cx="100" cy="90" r="85" class="fill-slate-800/80 stroke-slate-700/50 stroke-1" />

            <!-- Arco de progreso -->
            <path
                :d="arcPath"
                :stroke="speedColor"
                class="stroke-linecap-round fill-none stroke-[8] transition-colors duration-300"
            ...
            <g v-for="tick in ticks" :key="tick.value">
                ...
                <text
                    ...
                    class="fill-slate-300 text-xs font-bold"
                >
                    {{ tick.value }}
                </text>
            </g>
            ...
            <g class="transition-transform duration-300">
                ...
                />
                <circle cx="100" cy="90" r="4" :fill="speedColor" class="transition-colors duration-300" />
            </g>
            ...
            <!-- Valor digital -->
            <text x="100" y="120" text-anchor="middle" class="fill-white text-2xl font-bold">
                {{ Math.round(currentSpeed) }}
            </text>

            <!-- Unidad -->
            <text x="100" y="135" text-anchor="middle" class="fill-slate-400 text-sm">
