...
const form = useForm({
    pid: props.sensor.pid,
    name: props.sensor.name,
    description: props.sensor.description || '',
    category: props.sensor.category,
    unit: props.sensor.unit,
    data_type: props.sensor.data_type,
    min_value: props.sensor.min_value,
    max_value: props.sensor.max_value,
    requires_calculation: props.sensor.requires_calculation,
    calculation_formula: props.sensor.calculation_formula || '',
    data_bytes: props.sensor.data_bytes,
    is_standard: props.sensor.is_standard,
    notes: props.sensor.notes || '',
});

const showCustomCategory = ref(!props.categories.includes(props.sensor.category));
const customCategory = ref(showCustomCategory.value ? props.sensor.category : '');
...
const submit = () => {
    ...
    form.put(route('admin.sensors.update', props.sensor.id));
};
...
<template>
    <Head :title="`Editar ${sensor.name}`" />

    <AppLayout :breadcrumbs="breadcrumbs">
        <!-- Header -->
        <template #header>
            <div class="flex items-center space-x-4">
                <Link :href="route('admin.sensors.show', sensor.id)">
                ...
                <div class="flex items-center space-x-4">
                    ...
                    <div>
                        <h1 class="text-3xl font-bold text-gray-900 dark:text-gray-100">Editar Sensor</h1>
                        <p class="mt-1 text-gray-600 dark:text-gray-400">
                            <code class="rounded bg-gray-200 px-2 py-0.5 text-cyan-600 dark:bg-gray-700 dark:text-cyan-400">{{ sensor.pid }}</code>
                            {{ sensor.name }}
                        </p>
                    </div>
                </div>
                            ...
                            <div class="grid grid-cols-1 gap-6 md:grid-cols-3">
                                ...
                                <div class="md:col-span-3">
                                    ...
                                    <textarea
                                        ...
                                        rows="2"
                                        class="w-full rounded-lg border border-gray-300 px-3 py-2 font-mono text-sm focus:border-cyan-500 focus:ring-cyan-500 dark:border-gray-600 dark:bg-gray-800 dark:text-gray-200"
                                    ></textarea>
                                </div>
